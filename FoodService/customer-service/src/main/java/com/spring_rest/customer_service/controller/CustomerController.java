package com.spring_rest.customer_service.controller;

import com.spring_rest.customer_service.entity.Customer;
import com.spring_rest.customer_service.service.CustomerService;
...
public class CustomerController {
    ...
    @PostMapping("/add")
    @CrossOrigin(origins = {"http://localhost:3000", "https://webapp.glogistics.com"})
    public ResponseEntity<Map<String, String>> addCustomer(@RequestBody Customer customer) {
        ...
        try {
            ...
            if (StringUtils.hasText(customer.getContactNo()) && !isValidPhoneNumber(customer.getContactNo())) {
    ...
    private boolean isValidEmail(String email) {
        ...
        if (email.contains("<") || email.contains(">") || email.contains("script")) {
            return false;
        }

        String emailRegex = "^[A-Za-z0-9+_.-]+@([A-Za-z0-9.-]+\\.[A-Za-z]{2,})$";
        return email.length() <= 254 && email.matches(emailRegex);
    }
    ...
    private boolean isValidPhoneNumber(String phoneNumber) {
        ...
        return cleanPhone.matches("\\d{7,15}") && !phoneNumber.contains("<") && !phoneNumber.contains(">");
    }
}